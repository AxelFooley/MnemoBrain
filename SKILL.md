...
Creates the directory layout and `$MNEMOBRAIN_HOME/config/mnemobrain.yaml`.
Idempotent: re-running prints `init: config unchanged` when nothing changed
...
./.venv/bin/mnemobrain init   # persists the new values into the config file
```
...
"$GBRAIN" put mnemobrain-smoke --title "smoke" --body "mnemobrain smoke test" --home "$GBRAIN_HOME"
"$GBRAIN" get mnemobrain-smoke --home "$GBRAIN_HOME"   # must contain: mnemobrain smoke test
"$GBRAIN" rm mnemobrain-smoke --home "$GBRAIN_HOME"    # cleanup, exit 0
```
